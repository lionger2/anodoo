    ...
    def create(self, vals):
        ...
        if True:
            for opportunity in res:
                lifetime_id = opportunity.lifetime_id
                if lifetime_id:
                    activity_type_list = self.env['mail.activity.type'].search([('lifetime_id', '=', lifetime_id.id)], order='sequence')
                    
                    model_id = self.env['ir.model']._get(self._name).id
                    
                    for activity_type in activity_type_list:
                        date_deadline = fields.Date.context_today(self)
                        date_deadline = date_deadline + relativedelta(**{activity_type.delay_unit: activity_type.delay_count})
