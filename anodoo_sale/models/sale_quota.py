...
class SaleGoalModel(models.Model):
    ...
    dimension_field_ids = fields.One2many('anodoo.sale.goal.breakdown.dimension.field', 'goal_model_id', string='分解纬度对应字段')
...
class SaleGoal(models.Model):
    ...
    goal_model_id = fields.Many2one('anodoo.sale.goal.model', string='目标模型')
    
    goal_state_id = fields.Many2one('anodoo.sale.goal.state', string='目标状态')
    ...
    relation_ids = fields.One2many('anodoo.sale.goal.breakdown.dimension.relation', 'goal_id', string='目标分解', help='一个目标可以按照多个分解纬度来分解')

    
    
class SaleGoalState(models.Model):   
    _name = 'anodoo.sale.goal.state'
    _description = '销售目标状态'
    _rec_name = 'name'
    _order = "sequence, id"
...
class SaleGoalBreakdown(models.Model):   
    _name = 'anodoo.sale.goal.breakdown'
    _description = '销售目标分解'
    _rec_name = 'name'
    _order = 'id'
    _parent_store = True
    ...
    goal_id = fields.Many2one('anodoo.sale.goal', string='销售目标')
    
    dimension_id = fields.Many2one('anodoo.sale.goal.breakdown.dimension', string='分解纬度', domain=lambda self: self._get_dimensions_by_goal())
    ...
    parent_id = fields.Many2one('anodoo.sale.goal.breakdown', string='父分解', index=True, ondelete='cascade')
    child_ids = fields.One2many('anodoo.sale.goal.breakdown', 'parent_id', string='子分解')

    parent_path = fields.Char(index=True)
        
    owner_territory_id = fields.Many2one('anodoo.sale.territory', string='销售区域')
...
#                             ('day', '天'), ('workday', '工作日')]
    ...
    def _get_dimensions_by_goal(self):
        dimensions = []
        if self.goal_id :
            relations = self.env['anodoo.sale.goal.breakdown.dimension.relation'].search([('goal_id', '=', self.goal_id.id)])
...
class SaleGoalBreakdownDimension(models.Model):   
    _name = 'anodoo.sale.goal.breakdown.dimension'
    ...
    parent_id = fields.Many2one('anodoo.sale.goal.breakdown.dimension', string='父纬度', index=True, ondelete='cascade')
    child_ids = fields.One2many('anodoo.sale.goal.breakdown.dimension', 'parent_id', string='子纬度')
...
class SaleGoalBreakdownDimensionRelation(models.Model):   
    _name = 'anodoo.sale.goal.breakdown.dimension.relation'
    _description = '销售目标,分解纬度,分解三者的关系,一个销售目标支持几个分解纬度,就有几个分解'
    _rec_name = 'dimension_id'
    _order = 'id'    
    
    goal_id = fields.Many2one('anodoo.sale.goal', string='销售目标', required=True)
    
    dimension_id = fields.Many2one('anodoo.sale.goal.breakdown.dimension', string='分解纬度', required=True, domain="[('child_ids', '=', False)]")
    
    breakdown_id = fields.Many2one('anodoo.sale.goal.breakdown', string='目标分解')
    
class SaleGoalBreakdownDimensionField(models.Model):   
    _name = 'anodoo.sale.goal.breakdown.dimension.field'
    ...
    dimension_id = fields.Many2one('anodoo.sale.goal.breakdown.dimension', string='分解纬度')    
...
class SaleGoalBreakdownResult(models.Model):   
    _name = 'anodoo.sale.goal.breakdown.result'
    _description = '销售目标分解的考核结果'
    _rec_name = 'goal_breakdown_id'
    _order = 'id'
    
    goal_breakdown_id = fields.Many2one('anodoo.sale.goal.breakdown', string='目标分解')
